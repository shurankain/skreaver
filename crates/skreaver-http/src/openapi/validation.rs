...
use super::{ApiError, ValidationError, ApiResponse};
...
impl RequestValidator {
    ...
    pub fn validate_body(&self, body: &Value, schema_name: &str) -> Result<(), ValidationErrors> {
        ...
        let schema = self.schemas.get(schema_name)
            .ok_or_else(|| ValidationErrors::new(vec![
                ValidationError {
    ...
    pub fn validate_parameters(&self, params: &HashMap<String, String>, schema: &Value) -> Result<(), ValidationErrors> {
        ...
        let params_json: Value = params.iter()
            .map(|(k, v)| (k.clone(), Value::String(v.clone())))
            .collect::<serde_json::Map<_, _>>()
            .into();
        
        if let Err(validation_errors) = self.validate_against_schema(&params_json, schema, "parameters") {
    ...
    fn validate_against_schema(&self, value: &Value, schema: &Value, path: &str) -> Result<(), ValidationErrors> {
        let mut errors = Vec::new();
        ...
        if let (Value::Object(obj), Some(Value::Array(required))) = (value, schema.get("required")) {
            for required_field in required {
                if let Value::String(field_name) = required_field && !obj.contains_key(field_name) {
                    errors.push(ValidationError {
                        field: if path.is_empty() { field_name.clone() } else { format!("{}.{}", path, field_name) },
        ...
        if let Value::String(s) = value {
            if let Some(min_length) = schema.get("minLength").and_then(|v| v.as_u64()) && (s.len() as u64) < min_length {
                errors.push(ValidationError {
                    field: path.to_string(),
            ...
            if let Some(max_length) = schema.get("maxLength").and_then(|v| v.as_u64()) && (s.len() as u64) > max_length {
                errors.push(ValidationError {
                    field: path.to_string(),
            ...
            // Pattern validation
            if let Some(pattern) = schema.get("pattern").and_then(|v| v.as_str()) && let Ok(regex) = regex::Regex::new(pattern) && !regex.is_match(s) {
                errors.push(ValidationError {
                    field: path.to_string(),
        ...
        if let Value::Number(n) = value {
            if let Some(minimum) = schema.get("minimum").and_then(|v| v.as_f64()) && let Some(num_val) = n.as_f64() && num_val < minimum {
                errors.push(ValidationError {
                    field: path.to_string(),
            ...
            if let Some(maximum) = schema.get("maximum").and_then(|v| v.as_f64()) && let Some(num_val) = n.as_f64() && num_val > maximum {
                errors.push(ValidationError {
                    field: path.to_string(),
        ...
        if let Value::Array(arr) = value {
            if let Some(min_items) = schema.get("minItems").and_then(|v| v.as_u64()) && (arr.len() as u64) < min_items {
                errors.push(ValidationError {
                    field: path.to_string(),
            ...
            if let Some(max_items) = schema.get("maxItems").and_then(|v| v.as_u64()) && (arr.len() as u64) > max_items {
                errors.push(ValidationError {
                    field: path.to_string(),
...
impl ResponseValidator {
    ...
    pub fn validate_response(&self, body: &Value, schema_name: &str) -> Result<(), ValidationErrors> {
...
mod tests {
    ...
    fn test_request_validator_type_validation() {
        ...
        assert!(validator.validate_against_schema(&valid_value, &schema, "test").is_ok());
    ...
    fn test_request_validator_required_fields() {
        ...
        assert!(validator.validate_against_schema(&valid_value, &schema, "").is_ok());
    ...
    fn test_response_validator() {
        ...
        assert!(validator.validate_response(&valid_response, "response").is_ok());
